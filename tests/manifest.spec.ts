...
assert.ok(manifest.icons?.some((icon) => icon.sizes === "192x192" && icon.src === "/icons/icon-192.svg"));
assert.ok(manifest.icons?.some((icon) => icon.sizes === "512x512" && icon.purpose?.includes("maskable")));

for (const icon of manifest.icons ?? []) {
  assert.ok(icon.src, "Cada icono declara src");
  assert.equal(existsSync(resolve(root, `public${icon.src}`)), true, `Existe ${icon.src}`);
}

const layout = readText("src/app/layout.tsx");
assert.match(layout, /manifest:\s*"\/manifest\.webmanifest"/);
assert.match(layout, /themeColor:\s*"#1f6f5b"/);

const page = readText("src/app/page.tsx");
assert.match(page, /<AppShell inspections=\{inspections\} status="ready" \/>/);

const appShell = readText("src/components/app-shell.tsx");
assert.match(appShell, /<nav className="primary-nav" aria-label="Navegacion principal">/);
...
assert.match(appShell, /Cargando inspecciones/);
assert.match(appShell, /No se pudo cargar la lista/);
assert.match(appShell, /Sin inspecciones por mostrar/);
